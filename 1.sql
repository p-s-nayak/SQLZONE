...
--select date,count(*) as uniquie_date_count from unique_users group by date

select t1.date,
	case when t2.date is null then 0 else t2.uniquie_date_count end as uniquie_date_count
from(
select distinct(date) 
from user_activity )t1 
left join (
	select date,count(*) as uniquie_date_count 
	from unique_users 
	group by date) t2 
on t1.date = t2.date
